...
pub fn take(
  ...
  let overlapping: Vec<Lease> =
    active_in(conn, Some(namespace), now_ms, ttl_minutes)?
      ...
      .collect();
  if let Some(own) = overlapping.iter().find(|l| l.agent_id == agent_id) {
    conn.execute(
  ...
  conn.execute(
    "INSERT INTO leases
       (id, namespace, resource, agent_id, note, taken_at,
        renewed_at, created_at)
     VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?6, ?6)",
    params![id, namespace, resource, agent_id, note, now],
  )?;
...
pub fn release(
  ...
  let held = active_in(conn, Some(namespace), now_ms, ttl_minutes)?
    .into_iter()
    .find(|l| l.resource == resource && l.agent_id == agent_id)
    .ok_or_else(|| {
      LeaseError::NotHeld(
...
pub fn renew_for_agent(
  ...
  let n = conn.execute(
    "UPDATE leases SET renewed_at = ?1
     WHERE agent_id = ?2 AND released_at IS NULL
       AND renewed_at >= ?3",
    params![
...
pub fn active_in(
  ...
  let mut sql = String::from(
    "SELECT id, namespace, resource, agent_id, note, taken_at,
...
mod tests {
  ...
  fn take_overlap_release_round_trip() {
    ...
    assert!(release(&conn, "project/x", "store", "agent-a", now, TTL).is_err());
  }
  ...
  fn activity_renews_and_ttl_expires() {
    ...
    assert_eq!(renew_for_agent(&conn, "agent-a", later, TTL).unwrap(), 1);
    ...
    assert_eq!(
      renew_for_agent(&conn, "agent-a", stale_time, TTL).unwrap(),
      0
    );
  }
