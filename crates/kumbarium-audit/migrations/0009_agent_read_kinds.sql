-- 0009: the agent read verbs. 'get' is fetch-by-id (D-016's
-- summaries-then-full-entries promise finally has its second
-- half) and serves content, so it is witnessed like recall and
-- feeds the same survival evidence. 'task_list' is the docket
-- read tool the docket design reserved for round two; the
-- trigger (no id-acquisition path for ordinary matters
-- mid-session) fired. Same rebuild dance as ever.
